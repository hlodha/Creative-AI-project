...
for epoch in range(opt.niter):
    for i, data in enumerate(dataloader, 0):
        ...
        #errG = (errG + (opt.lamb + 0.250*(epoch//3))*errG_class_fake)/(1.0* (1+0.250*(epoch/3)))
        errG = (errG + (0.1*(epoch//10))*errG_class_fake)/(1.0* (1+0.1*(epoch//10)))
        errG.backward()
        ...
        if(i%20 == 0):
            print('[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f D(x): %.4f D(G(z)): %.4f / %.4f, F_C_P = %.4f' % (epoch, opt.niter, i, len(dataloader),
                    errD.item(), errG.item(), D_x, D_G_z1, D_G_z2, fake_c_out.mean().item()))
        if i % 100 == 0:
